...
# API budget: fixture lists 2h (4h summer); enrich disk 8h; deep pass today + 5d window; reboost after cache clear.
HIBS_CACHE_TTL_HOURS=2
HIBS_CACHE_TTL_SUMMER_HOURS=4
HIBS_TARGET_DQ_PCT=90
HIBS_DEEP_ENRICH_MAX_RETRIES=3
HIBS_DEEP_ENRICH_TODAY_ONLY=1
HIBS_DEEP_ENRICH_WINDOW_DAYS=5
HIBS_DEEP_ENRICH_RESCUE_LOW=1
# Friendlies: standard enrich (85%+ DQ); max-data only when HIBS_FRIENDLIES_MAX_DATA=1
HIBS_FRIENDLIES_MAX_DATA=0
HIBS_ENABLE_FOTMOB_FRIENDLIES=1
HIBS_FRIENDLIES_STATISTICS_XG_RESERVE=8
HIBS_BUNDLE_DQ_REBOOST=1
HIBS_BUNDLE_DQ_REBOOST_MAX=64
...
# Tournament focus: unset = auto 2026-06-01..2026-07-18 (international default in window).
# HIBS_TOURNAMENT_FOCUS=0 only to force domestic during the window (rare).
HIBS_FBREF_BLOCKED=1
HIBS_FOOTBALL_DATA_PER_MIN_LIMIT=8
HIBS_FOOTBALL_DATA_TEAM_MATCHES=0
HIBS_FOOTBALL_DATA_STANDINGS_CACHE_HOURS=24
HIBS_FOOTBALL_DATA_HEALTH_PROBE_MIN=30
HIBS_PUBLIC_TRACKER=1
HIBS_SCALE_READY_CLV_PCT=60
HIBS_PROOF_CLV_BEAT_PCT=60
HIBS_PREDICTION_LOG_ENABLED=1
HIBS_PREDICTION_LOG_ALWAYS=1
HIBS_PREDICTION_LOG_MIN_INTERVAL_SEC=21600
...
echo "Cron (www-data — pred-log-sync + calibration-fit):"
echo "  sudo bash ${APP_ROOT}/deploy/cron-hibs-calibration.sh --install"
echo "Test:"
echo "  curl -sS --max-time 30 -o /dev/null -w 'health %{http_code}\n' http://127.0.0.1:8000/api/health"
echo "  curl -sS --max-time 30 -o /dev/null -w 'login %{http_code}\n' http://127.0.0.1:8000/login"
